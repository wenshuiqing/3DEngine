...
#pragma pack(pop)
	...
	class BmpParser :implements ImageParse {
	...
	public:
		...
		virtual Image Parser(const Buffer& buffer) {
			...
			Image img;
			BITMAP_FILEHEADER* pFileHeader = reinterpret_cast<BITMAP_FILEHEADER*>(buffer.GetData());
			BITMAP_HEADER* pBmpHeader = reinterpret_cast<BITMAP_HEADER*>(buffer.GetData() + BITMAP_FILEHEADER_SIZE);
			if (pFileHeader->Signature == 0x4D42 /* 'B''M' */) {
				...
				if (img.bitcount < 24) {
					...
					uint8_t* pSourceData = buffer.GetData() + pFileHeader->BitsOffset;
